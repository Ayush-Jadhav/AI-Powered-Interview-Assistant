...
const ResumeUploader = () => {
  ...
  const extractInfo = (text) => {
    ...
    // 1. find capitalized name (Firstname Lastname) at beginning of the text
    // more better technique than just grabbing first line.
    const nameMatch = text.match(/^\s*([A-Z][a-z]+(?:\s[A-Z][a-z]+)+)/);
    if (nameMatch) {
      name = nameMatch[0].trim();
    } else {
      // 2. As a fallback, use original method.
      // This could still be useful for resumes with unusual formatting.
      name = text.split("\n")[0].trim();
      // 3. If fallback grabs too much text (likely the whole resume), clear it.
      if (name.length > 50) {
        name = "";
      }
    }
  ...
  const handleFileUpload = async (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        message.error(
          "Failed to parse PDF. Please fill in the details manually."
        );
      }
    };
    return false;
  ...
  return (
    <Card title="Start Your Interview">
      ...
      <p style={{ margin: "1rem 0", color: "#888" }}>
        We'll try to extract your Name, Email, and Phone. Please verify and fill
        any missing fields.
      </p>
