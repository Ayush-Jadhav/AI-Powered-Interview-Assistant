...
const InterviewChat = () => {
  ...
  const { activeInterview, candidates } = useSelector(
  ...
  // state to explicitly control timer
  ...
  const timeLeft = useInterviewTimer(
    isTimerActive ? getTimerDuration() : 0,
    handleTimeUp
  );
  ...
  useEffect(() => {
    ...
    const activeCandidate = candidates.find(
      (c) => c.id === activeInterview.candidateId
    );
    if (qIndex >= TOTAL_QUESTIONS) {
      if (activeCandidate && activeCandidate.status !== "completed") {
        setIsLoading(true);
        const totalScore = activeCandidate.questions.reduce(
          (acc, q) => acc + q.score,
          0
        );
        const finalScore =
          (totalScore / (activeCandidate.questions.length * 10)) * 10;

        generateFinalSummary({ ...activeCandidate, finalScore }).then(
          (summary) => {
            dispatch(
              completeInterview({
                finalScore,
    ...
    generateQuestion(
      qIndex,
      activeCandidate?.questions.map((q) => q.question) || []
    ).then((q) => {
        ...
        <>
          ...
          <div style={{ margin: "1rem 0" }}>
            <Progress
              ...
              percent={
                isTimerActive
                  ? Math.round((timeLeft / getTimerDuration()) * 100)
                  : 100
              }
              format={() => `${timeLeft}s`}
            />
          ...
          <TextArea
            rows={6}
            value={answer}
            onChange={(e) => setAnswer(e.targe.value)}
